...
- Each Project has one JSON configuration document, three always-required conventionally named Markdown prompts for implementation, CI repair, and conflict repair, an enabled-only documentation prompt, and its own log directory. Prompt names and locations are fixed rather than configurable. The exact agreed layout and CLI spelling remain recorded in [decision #6](https://github.com/xiakeng/sandcastle-runner/issues/6#issuecomment-5592352688).
- Configuration includes `repository` in owner/repository form; absolute `checkout`; optional `targetBranch`; `tracker.type`, `tracker.tokenEnv`, `tracker.runnerAccount`, and `tracker.reservationLabel`; `codeHost.type`, `codeHost.tokenEnv`, and `codeHost.adminMerge`; `workflow.documentationMaintenance`; `agents.implement`, `agents.ciRepair`, and `agents.conflictRepair`; conditionally required `agents.documentation`; `timeouts.agentMinutes`, `timeouts.requiredChecksMinutes`, and `timeouts.mergeQueueMinutes`; and `ticketClosure`. Agent profiles contain `model` and `reasoningEffort`.
- `workflow.documentationMaintenance` is a strict boolean and defaults to `true` when its field or the workflow object is omitted. The workflow object rejects unknown fields except `$comment`; `review` is a recognized sibling switch whose execution contract belongs to its review delivery. Enabling Documentation Maintenance requires both `agents.documentation` and a nonempty `prompts/documentation.md`; startup errors identify both paths. Disabling it permits both to be omitted and does not read the prompt, while a supplied profile is still validated.
