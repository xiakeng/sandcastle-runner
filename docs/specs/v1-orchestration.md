...
- The CLI provides a `run` command requiring `--project` and exactly one of `--parent` or `--issue`; both values are GitHub issue numbers and the project key selects that Project's directory under the Runner repository.
...
- A Run records its recovery snapshot as JSON, including publication identities, phase, Target Branch and remote-base facts, pending pushes, fresh repair Worktree/branch/head evidence, and independent CI/conflict repair budgets with consumed attempts and operator-authorized generations. Restart resumes only from exact persisted evidence; an interrupted push is adopted only when the remote head matches the recorded intended head, while unexpected advances pause without overwrite, rebase, or force-push.
- Recovery deliberately excludes old-process management, adoption of missing snapshots, and unrequested recovery state matrices. The audit log remains diagnostic only and is never recovery input.
