...
restart. Publication records retain the original Target Branch, stable remote delivery branch,
intended head SHA, exact PR metadata, and review evidence. A pending push is adopted only when the
remote branch head matches exactly; absence restarts from a newly fetched base, while an unexpected
head pauses. Interrupted PR creation adopts one exact repository/branch/base/head match, creates
only after authoritative absence, and never rewrites existing PR metadata.
