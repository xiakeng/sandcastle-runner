...
import {
  ...
  sha,
  ticketForBranch,
} from "./full-contract.ts";

export type RestartPoint = "publication" | "integration" | "maintenance" | null;

export function createFiveTicketScenario(
  root: string,
  recovery: boolean,
    ...
    agentExecutor: {
      async execute(input) {
        ...
        try {
          ...
          const attemptResult = makeAttemptResult(input, commit);
