...
test("an unusable read override pauses again and never enters the audit log", async () => {
  const root = await createProject();
  let parentCalls = 0;
  const responses = ["{}", '{"number":8,"state":"open","stateReason":null}'];
  const result = await executeCli(
    ["run", "--project", "demo", "--parent", "8"],
    {
      root,
  ...
  const result = await executeCli(
    ["run", "--project", "demo", "--parent", "8"],
    {
      ...
      operator: {
        write() {},
        async pause() {
          throw new Error("startup failures must not pause");
        },
      },
    },
  );
...
test("invalid CLI arguments return a structured startup failure", async () => {
  let externalCalls = 0;
  const result = await executeCli(["run", "--project", "demo"], {
    root: "/unused",
    env: {},
    tracker: {
      async getParent() {
    ...
    clock: { now: () => new Date(), async sleep() {} },
    operator: {
      write() {},
      async pause() {
        externalCalls += 1;
  ...
  const result = await executeCli(
    ["run", "--project", "demo", "--parent", "8"],
    {
      ...
      operator: {
        write() {},
        async pause() {
          throw new Error("no pause expected");
        },
      },
    },
  );
...
test("unsupported model and effort selections fail before workflow operations", async () => {
  for (const implement of [
    { model: "not-a-codex-model", reasoningEffort: "high" },
